...
class NERModel(object):
    def __init__(self, config, embeddings, uni_embeddings, pos_embeddings, ntags, nchars=None, vocab_words=None,
                 NE_dic = None):
        """
        Args:
        ...
        self.uni_embeddings = uni_embeddings
        ...
        self.NE_dic = NE_dic
        self.pconfig = projector.ProjectorConfig()
    ...
    def add_placeholders(self):
        ...
        self.dic = tf.placeholder(tf.float32, shape=[None, None, 4],
                                  name = "dic")
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("words"):
            ...
            ####
            '''_uni_word_embeddings = tf.Variable(self.uni_embeddings, name="_uni_word_embeddings", dtype=tf.float32,
                                           trainable=self.config.uni_train_embeddings)

            uni_word_embeddings = tf.nn.embedding_lookup(_uni_word_embeddings, self.word_ids,
                                                     name="uni_word_embeddings")
            word_embeddings = tf.concat([word_embeddings, uni_word_embeddings], axis=-1)
            '''

            ### NE_dic

            '''_NE_dic = tf.Variable(self.NE_dic, name="NE_dic", dtype=tf.float32,
                                               trainable=self.config.dic_train_embeddings)

            _NE_dic = tf.nn.embedding_lookup(_NE_dic, self.word_ids,
                                                         name="ne_dic")

            word_embeddings = tf.concat([word_embeddings, _NE_dic], axis=-1)
            '''
            ### POS_embedding

            _pos_embeddings = tf.Variable(self.pos_embeddings, name="_pos_embeddings", dtype=tf.float32, trainable=self.config.pos_train_embeddings)

            pos = tf.nn.embedding_lookup(_pos_embeddings, self.pos_ids, name="word_embeddings")
            

            word_embeddings = tf.concat([word_embeddings, pos], axis=-1)

            self.w_e = word_embeddings
            ######

            ### uni, bi, trigram
            pprint(self.embeddings)
            embedding = self.pconfig.embeddings.add()
            embedding.tensor_name = _word_embeddings.name
    ...
    def add_summary(self, sess):
        ...
        self.file_writer = tf.summary.FileWriter(self.config.tensorboard_log_path, sess.graph)
    ...
    def predict_batch(self, sess, words, pos):
        """
        Args:
        ...
        fd, sequence_lengths = self.get_feed_dict(words=words,pos=pos, dropout=1.0)
    ...
    def run_epoch(self, sess, train, dev, tags, epoch):
        ...
        #_, _, _, _, p = minibatches(train, self.config.batch_size)

        #rint("len {}".format(p))

        for i, (words, labels, pos, _, _ ) in enumerate(minibatches(train, self.config.batch_size)):
            fd, _ = self.get_feed_dict(words=words, pos=pos, labels=labels, lr=self.config.lr, dropout=self.config.dropout)

            _, train_loss, summary = sess.run([self.train_op, self.loss, self.merged], feed_dict=fd)

            #eb = sess.run(self.w_e, feed_dict=fd)
       ...
       # acc2, f2 = self.run_evaluate(sess, train, tags)
        #self.logger.info("- train acc {:04.2f} - f1 {:04.2f}".format(100 * acc2, 100 * f2))
    ...
    def run_evaluate(self, sess, test, tags):
        ...
        accs = []
        f = open('/home/chiyoon/python/seq/sequence_tagging/predict/prediction.txt','w')
        f2 = open('/home/chiyoon/python/seq/sequence_tagging/predict/output.txt','w')

        correct_preds, total_correct, total_preds = 0., 0., 0.
        for words, labels, pos, inputs, position in minibatches(test, self.config.batch_size):
            labels_pred, sequence_lengths = self.predict_batch(sess, words, pos)

            for lab, lab_pred, length, wo, idx in zip(labels, labels_pred, sequence_lengths, inputs, position):
                lab = lab[:length]
                ...
                for (w, po,  pre, label) in zip(skip_w, skip_idx, _predict, _label):
                    if pre == label:
                        f2.write("{}\t{}\t{}\n".format(w, pre, label))
                    else:
                        f2.write("{}\t{}\t{} <<<<<<<<<<<<<<<<<<<<<<<<<<\n".format(w, pre, label))
            ...
            while True:
                try:
                    ...
                    if type(words[0]) == tuple:
                        words = zip(*words)
                    print("go batch")
                    pred_ids, _ = self.predict_batch(sess, [words], [positions])
